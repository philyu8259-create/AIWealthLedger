...
2. 酒店/火车票/机票/门票/旅行订单截图：
   - 每个"已确认/出票成功/已购/已支付"的真实订单输出一条
   - 酒店按"实付款/总价"输出住宿类或旅行类一条；火车票/机票按"合计/票价/实付"输出交通类一条
   - 推荐景点、日期、电话、订单号、车次、房型、图片说明不是账单，必须忽略
3. 超市/餐厅/普通购物小票：
   - 如果有清晰商品明细行（商品名 + 金额），可按每行商品输出多条
   - 小计、税费、折扣、总计只用于校验，不要和商品明细重复输出
4. 支付截图：
   - 一张支付成功截图通常只输出一条，以"付款金额/支付金额/实付"为准
5. 无论哪种场景，每笔真实交易只输出一个金额，不要把税费、折扣、日期、订单号识别成账单
